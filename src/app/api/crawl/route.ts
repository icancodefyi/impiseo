...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const page of pending) {
      // One page's failure must never abort the whole batch. Mark the in-flight
      // page as failed so it becomes crawlable again instead of silently dropping.
      try {
        ...
        failed++;
        errors.push(`${page.path}: ${err instanceof Error ? err.message : "crawl failed"}`);
        await pages.updateOne(
          { _id: page._id },
          { $set: { crawledAt: new Date(), crawlError: err instanceof Error ? err.message : "crawl failed" } }
        );
      }
